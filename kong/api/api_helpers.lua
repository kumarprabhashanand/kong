  ...
  for route_path, methods in pairs(routes) do
    ...
    for method_name, method_handler in pairs(methods) do
      ...
      if HTTP_METHODS[method_name] then
        ...
        methods_array[methods_count] = method_name
      end
    end
    ...
    if not methods["HEAD"] and methods["GET"] then
      ...
      methods_count = methods_count + 1
      methods_array[methods_count] = "HEAD"
    end

    if not methods["OPTIONS"] then
      methods_count = methods_count + 1
      methods_array[methods_count] = "OPTIONS"
      table.sort(methods_array)
      methods["OPTIONS"] = options_method(table.concat(methods_array, ", ", 1, methods_count))
    end
